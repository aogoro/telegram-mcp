...
type ToolRetType struct {
	...
	// Optional hints about tool behavior (MCP spec tool annotations).
	Annotations *ToolAnnotations `json:"annotations,omitempty" yaml:"annotations,omitempty" mapstructure:"annotations,omitempty"`
}

// ToolAnnotations are optional hints about tool behavior, per the MCP spec.
// Clients (e.g. Claude Code plan mode) use ReadOnlyHint to decide whether a
// tool may run without confirmation.
