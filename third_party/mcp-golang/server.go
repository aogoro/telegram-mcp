...
// SetToolAnnotations attaches MCP tool annotations (e.g. readOnlyHint) to an
// already registered tool; they are surfaced in tools/list responses.
func (s *Server) SetToolAnnotations(name string, annotations *ToolAnnotations) error {
	t, ok := s.tools.Load(name)
	if !ok {
...
func (s *Server) handleListTools(ctx context.Context, request *transport.BaseJSONRPCRequest, _ protocol.RequestHandlerExtra) (transport.JsonRpcBody, error) {
	...
	for i := startPosition; i < endPosition; i++ {
		toolsToReturn = append(toolsToReturn, ToolRetType{
			...
			Annotations: orderedTools[i].Annotations,
		})
	}
